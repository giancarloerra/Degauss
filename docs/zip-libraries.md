...
`--report` and `--audit` list skipped members and skipped archives as `unreadable` lines with their reason. When `--report` has to prepare a system's Artwork Pack cache first, the scan's warnings are printed as `source note` lines.
