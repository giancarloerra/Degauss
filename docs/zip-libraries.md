...
Every skipped member is written to `/tmp/degauss.log` as `zip <archive>: member <name>: <reason>` (a name holding a control character is escaped there) whenever a listing reads the archive, once per read: a rebuild, `--report`, `--audit` and a listing straight from the card of a system that is not indexed all leave the lines. The check before a launch and the resolution of a favourite naming a member read the archive without writing them: the launch check reports the chosen member's reason in its own message, and a start with several favourites in one archive reads it once per favourite, which would otherwise repeat the block each time. A skipped archive is written by the scan as `zip <archive>: skipped: <error>` at the moment it is skipped, and a folder skipped for its depth as `zip <archive>/<folder>: skipped: <reason>`.
