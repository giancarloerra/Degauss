...
A system that does not list `zip` among its launchable extensions reads each ZIP as a virtual library. When it contains exactly one member the system can launch, that member appears directly as a game in the ZIP's containing folder, including when it sits in nested archive directories or beside unsupported companion files. Its native `archive.zip/folder/game.ext` target is unchanged; neither browsing nor launch preparation extracts or modifies the archive. A ZIP containing several supported games remains a virtual folder, with directories taken from explicit ZIP records or implied by member paths. A system already configured to launch `zip` keeps the whole archive as one game. A multi-game archive, or a virtual directory inside one, holding exactly one game with artwork, whatever else without a picture sits beside it, or several members that all share one picture, shows that artwork on its folder row before it is opened.
...
A Neo Geo or Neo Geo MVS ZIP whose name, without the extension, matches an entry of the applicable `romsets.xml` (the one in the folder being listed, else the one at the top of the first declared game folder, which MiSTer Main treats as the core's home folder) is one game rather than a virtual folder. It is listed under the catalogue's title (a ZIP whose entry carries the `hide` attribute is not listed at all), is never opened, extracted or validated to build the list, and its complete path is handed to Main in the `.neo` file slot; Main's ROM-set loader then reads the components. A favorite for it is an ordinary MGL naming the complete ZIP path. Gamelist metadata binds to `./setname.zip`, and an Artwork Pack matches the set name without hashing. A ZIP the catalogue does not name, in those systems or any other, keeps the generic archive behavior described above. A `romset.xml` member inside a ZIP is not read, because that would mean opening the archive to list it.
