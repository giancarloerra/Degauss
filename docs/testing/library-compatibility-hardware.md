...
2. Prepare through the actual Artwork Pack UI: under Automatic, enter the system and answer **Prepare** to the Artwork Pack Available question; with an explicit Artwork Pack choice, no question is asked. Verify names and distinct artwork, then restart and verify the saved cache is reused without a preparation overlay.
...
5. Introduce a malformed descriptor before its identity and rebuild. Verify the preparation completes with problems, that the malformed descriptor's game stays listed with its filesystem name and no Pack data, that `/tmp/degauss.log` names it under `pack entry`, and that the summary and every other entry are unchanged. Restore it and rebuild again to verify it is matched.
