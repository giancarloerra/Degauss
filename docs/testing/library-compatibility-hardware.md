...
10. **Storage and upgrade:** repeat the listing checks from USB and a mounted network path when available, and verify a 0.6.0 installation keeps its settings, positions and favorites after rebuilding the Neo Geo list and the Neo Geo MVS list once each (they are cached separately) or after one full rebuild.
