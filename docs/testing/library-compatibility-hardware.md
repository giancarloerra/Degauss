...
8. **Favorites:** favorite one set of each format, restart, check the hearts, launch each favorite, and open the same favorite from the stock menu. Include a folder known only by its own `romset.xml` and record whether Main starts it from the absolute path the favorite carries; Main resolves `<root>/<path>/romset.xml` and falls back to the catalogue when that file is absent. Write one favorite by hand with a bare set name (`mslug` or `mslug.zip`, no folder) for a set folder and for a set ZIP; both must launch from Degauss and from the stock menu. With the shipped table neither carries a heart on its row, because Neo Geo and Neo Geo MVS both claim the name.
