...
fn run_automatic_pack_consent_flow(root: &Path, window: Rc<MinimalSoftwareWindow>) {
    ...
    let state = crate::cache::load_pack_source_state(&app.cache_dir, "NES").unwrap();
    ...
    enrich_favorite_rows(
        ...
        &roots,
        &cache_dir,
        |id, root, _| app.provider_from_state(id, root),
    );
    ...
    // 13. The language changed: asked; 15. Update replaces the result
    // after success, for this system alone.
    leave_system(&mut app);
    write_gameinfo("Pack First");
    ...
    let updated_files = pack_files(&app.cache_dir, "NES");
    ...
    // Rebuild All keeps the accepted system prepared, as a visible part of
    // the rebuild, and leaves the decision as it was.
    app.message = None;
    app.start_build(true);
    assert_eq!(
        app.source_recovery_queue
            .iter()
            .map(String::as_str)
            .collect::<Vec<_>>(),
        ["NES"]
    ...
    // Rebuild All prepares the accepted member inside the rebuild and
    // reads the other one the ordinary way.
    leave_system(&mut app);
    app.start_build(true);
    app.finish_background_work_for_headless();
    assert_eq!(app.index_terminal.as_ref().unwrap().state, "Complete");
...
fn run_arcade_descriptor_failures_flow(root: &Path, window: Rc<MinimalSoftwareWindow>) {
    ...
    let state = crate::cache::load_pack_source_state(&app.cache_dir, "Arcade").unwrap();
    ...
    assert!(!log_since(logged_from).contains("pack entry"));
