...
pub enum Event {
    ...
    Staged {
        ...
        progress: Progress,
        /// Content the scan left out, including unreadable descriptors,
        /// unsupported archives or members, and unreadable Neo Geo
        /// catalogues. Each summary is prefixed with the system name. The
        /// group still stages: one broken item is not a reason to fail every
        /// healthy row in it.
        warnings: Vec<String>,
    },
    Cancelled(Progress),
